...
//! |---|---|---|
//! | Buffer aligned to 16 bytes | `IsBufferAlignedTo16` | Type-level guard |
//! | Workgroup size divides thread count | `IsWorkgroupSizeDivisible` | Type-level guard |
//! | Dispatch within device limits | `IsDispatchWithinLimits` | Type-level guard |
//! | Kernel output is deterministic | `IsMSLKernelDeterministic` | Statistical (amari-flynn) |
//!
//! # Phase
//!
...
pub use karpal_proof::Property;
pub use karpal_proof::Proven;
...
/// Properties asserted:
/// - Input and output buffers are 16-byte aligned
/// - Thread count (1024 × 256) is divisible by workgroup size (256)
/// - Dispatch fits within Metal's 1D grid limit
/// - Kernel is deterministic across dispatches
...
///
/// Properties asserted:
/// - All three buffers are 16-byte aligned
/// - Thread count (1024 × 256) is divisible by workgroup size (256)
/// - Dispatch fits within Metal's 1D grid limit
/// - Kernel is deterministic across dispatches
